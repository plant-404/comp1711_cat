...
int main()
{
    ...
    // Create array of structs to store data for manipulation
    reading* arrayReadings;
    int lenReadings = read_file(input, &arrayReadings);
    // Close file, free resources, no longer needed as data in structs
    fclose(input);

    // Check for bad data, error if bad
    if(data_checker(arrayReadings, lenReadings)){
        printf("Bad data found in file.\n");
        return 1;
    }
    ...
    while (1)
    {
        ...
        case 'a':
            ...
            {
                printf("%s - Blood iron: %.1f\n", arrayReadings[i].date, arrayReadings[i].bloodIron);
            }
            break;
        ...
        case 'Q':
        case 'q':
            // Free the struct of arrays - prevent mem leak
            free(arrayReadings);
            return 0;
            break;
